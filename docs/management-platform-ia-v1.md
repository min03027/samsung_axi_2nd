...
- 현재 구현에서는 `ADMIN`이 하나의 관리자 Role이며 `AdminLevel`은 없다.
- 문서의 운영관리자/최고관리자 구분은 향후 권한 설계 용어다. 현재는 로그인 아이디 기반 `SuperAdminPolicy`가 일부 자격증명 변경만 예외적으로 제한한다.
...
현재 코드에는 운영관리자 또는 최고관리자라는 별도 Role·DB 등급이 없고 모두 `Role.ADMIN`이다.

- 전체 또는 배정된 복수 과정 운영
- 학생·강사·과정, 출결·이수, 평가, 상담, 공지·설문
- 통합 리포트, 취업·사후관리, 데이터 연계
- 일반 `/admin/**` 접근은 동일한 ADMIN 권한으로 처리

### SuperAdminPolicy 특례 — 현재 구현

- 별도 Role이나 관리자 등급이 아니다.
- `SuperAdminPolicy`가 설정된 로그인 아이디(기본값 `admin`)에만 계정 로그인 정보 변경 기능을 허용한다.
- 그 외 일반 관리자 기능은 같은 `Role.ADMIN` 경계에 있으며, 이 특례를 전체 최고관리자 권한 모델로 해석하지 않는다.

### 운영관리자/최고관리자 — 향후 설계

- 향후 필요 시 `ADMIN` 내부에서 `AdminLevel + Permission + DataScope`로 구분한다.
- 이 구분은 아직 구현되지 않았으며 현재 메뉴·API 권한을 설명하는 용어로 사용하지 않는다.
